...
class CalculatorViewModel: ViewModel() {
    ...
    fun onClick(action: CalculatorAction){
        when(action){
            ...
            CalculatorAction.Delete -> performDeleteion()
    ...
    private fun performCalculation() {
        ...
        if (number1!= null && number2!=null){
            ...
            val modifiedResult = if (result.toString().endsWith(".0")) {
                result.toInt()
            } else {
                result
            }

            state = state.copy(number1 = modifiedResult.toString().take(10), number2 = "", operation = null)
        }
